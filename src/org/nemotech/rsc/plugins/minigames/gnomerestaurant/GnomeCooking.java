...
public class GnomeCooking extends Plugin implements InvActionListener, InvActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener {
    ...
    enum GnomeCook {
        ...
        GNOME_BOWL_ALREADY_COOKED(885, 885, 888, 30, 1,
                "You cook the gnome bowl in the oven...",
                "You remove the gnome bowl from the oven",
                "You accidentally burn the gnome bowl");
    ...
    public boolean canCook(InvItem item, GameObject object) {
    ...
    private void handleGnomeCooking(final InvItem item, Player p, final GameObject object) {
        GnomeCook gc = null;
        for(GnomeCook c : GnomeCook.values()) {
            if(item.getID() == c.uncookedID && inArray(object.getID(), 119)) {
                gc = c;
        ...
        if (p.getInventory().remove(item) > -1) {
            ...
            if (!burnFood(p, gc.requiredLevel, p.getCurStat(COOKING))) {
                if(gc.cookedID == 884) {
                ...
                p.getInventory().add(new InvItem(gc.burntID));
    ...
    private boolean mouldDough(InvItem item, Player p) {
        ...
        if(menu != -1) {
            ...
            p.incExp(COOKING, (int) 25.0, true);
            p.setBusy(false);
        }
        return true;
