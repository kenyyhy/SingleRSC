...
import static org.nemotech.rsc.plugins.Plugin.THE_HOLY_GRAIL;
...
public class TheHolyGrail implements QuestInterface,TalkToNpcListener,
    ...
    public void handleReward(Player player) {
        ...
        player.incQuestExp(5, (player.getMaxStat(5) + 1) * 250);
        player.incQuestExp(1, (player.getMaxStat(1) + 1) * 300);
    }
    ...
    public void onWallObjectAction(GameObject obj, Integer click, Player p) {
        if (obj.getID() == 117) {
            if (p.getQuestStage(this) >= 1
                    && atQuestStage(p, Plugin.MERLINS_CRYSTAL, -1)
        ...
        if (obj.getID() == 116) {
            ...
            if (p.getInventory().countId(738) != 2
                    && (p.getQuestStage(THE_HOLY_GRAIL) >= 3 || p
                            .getQuestStage(THE_HOLY_GRAIL) == -1)) {
