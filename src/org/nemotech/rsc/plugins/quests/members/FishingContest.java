...
import static org.nemotech.rsc.plugins.Plugin.FISHING_CONTEST;
...
public class FishingContest implements QuestInterface,TalkToNpcListener,
    ...
    public boolean blockInvUseOnObject(final GameObject obj,
            final InvItem item, final Player player) {
        if (obj.getID() == 355) {
            return true;
        }
        if (obj.getID() == 350) {
    ...
    public void handleReward(final Player p) {
        p.updateQuestStage(Plugin.FISHING_CONTEST, -1);
        p.message("Well done you have completed the fishing competition quest");
        p.message("@gre@You have gained 1 quest point!");
        p.incQuestPoints(1);
        if(p.getMaxStat(10) <= 23) {
            p.incQuestExp(10, (p.getMaxStat(10) - 10) * 75 + 975);
        } else if(p.getMaxStat(10) >= 24) {
            p.incQuestExp(10, (p.getMaxStat(10) - 24) * 75 + 2225);
        }
    }
    ...
    private void mountainDwarfDialogue(final Player p, final NPC n) {
        ...
        case 3:
            ...
            if (hasItem(p, 720)) {
                ...
                p.sendQuestComplete(FISHING_CONTEST);
            } else {
    ...
    @Override
    public void onInvUseOnObject(final GameObject obj, final InvItem item,
            final Player player) {
        ...
        if (obj.getID() == 352) {
            ...
            if (bonzo != null && !p.getCache().hasKey("paid_contest_fee")) {
                ...
                if (hasItem(p, 715) && hasItem(p, 377)) {
                    ...
                    p.getInventory().add(new InvItem(717));
