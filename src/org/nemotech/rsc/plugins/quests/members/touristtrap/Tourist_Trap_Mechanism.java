package org.nemotech.rsc.plugins.quests.members.touristtrap;
...
import org.nemotech.rsc.plugins.listeners.action.UnwieldListener;
...
public class Tourist_Trap_Mechanism implements UnwieldListener, InvUseOnNpcListener, InvUseOnNpcExecutiveListener, ObjectActionListener, ObjectActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener, InvUseOnItemListener, InvUseOnItemExecutiveListener, DropListener, DropExecutiveListener, TalkToNpcListener, TalkToNpcExecutiveListener {
    ...
    public void onUnwield(Player player, InvItem item) {
        if((item.getID() == 1022 || item.getID() == 1023) && (player.getLocation().inTouristTrapCave()) && player.getQuestStage(Plugin.TOURIST_TRAP) != -1) {
    ...
    public boolean blockInvUseOnNpc(Player p, NPC npc, InvItem item) {
        if(item.getID() == TECHNICAL_PLANS && npc.getID() == BEDABIN_NOMAD_GUARD && p.getQuestStage(Plugin.TOURIST_TRAP) >= 7) {
            return true;
        }
    ...
    public void onInvUseOnNpc(Player p, NPC npc, InvItem item) {
        ...
        if(item.getID() == PINE_APPLE && npc.getID() == MERCENARY_INSIDE) {
            ...
            if(p.getQuestStage(Plugin.TOURIST_TRAP) == 8) {
                p.updateQuestStage(Plugin.TOURIST_TRAP, 9);
            }
        }
        if(item.getID() == 1038 && npc.getID() == ANA) {
            if(p.getQuestStage(Plugin.TOURIST_TRAP) == -1) {
    ...
    private void attachFeathersToPrototype(Player p, InvItem i, InvItem i2) {
        if(i.getID() == 381 && i2.getID() == 1071 || i.getID() == 1071 && i2.getID() == 381) {
            ...
            removeItem(p, 381, 10);
            p.incExp(FLETCHING, (int) 5.0, true);
        }
    }
    ...
    public void onObjectAction(GameObject obj, String command, Player p) {
        ...
        if(obj.getID() == MINING_CAVE) {
            ...
            if(!p.getInventory().wielding(1022) && !p.getInventory().wielding(1023) && p.getQuestStage(Plugin.TOURIST_TRAP) != -1) {
            ...
            if(p.getQuestStage(Plugin.TOURIST_TRAP) >= 9 || p.getQuestStage(Plugin.TOURIST_TRAP) == -1) {
                message(p, "You walk into the dark of the cavern...");
                p.message("And emerge in a different part of this huge underground complex.");
                p.teleport(76, 3640);
                return;
