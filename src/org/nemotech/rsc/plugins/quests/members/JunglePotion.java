...
import static org.nemotech.rsc.plugins.Plugin.setQuestStage;
import static org.nemotech.rsc.plugins.Plugin.showMenu;
import org.nemotech.rsc.plugins.Plugin;
import org.nemotech.rsc.model.GameObject;
import org.nemotech.rsc.model.NPC;
import org.nemotech.rsc.model.player.Player;
import static org.nemotech.rsc.plugins.Plugin.JUNGLE_POTION;
...
public class JunglePotion implements QuestInterface,ObjectActionListener,
...
WallObjectActionExecutiveListener {
    ...
    public void handleReward(Player player) {
        ...
        player.incQuestExp(15, (player.getMaxStat(15) * 125) + 400);
    ...
    private void trufitisChat(Player p, NPC n, int cID) {
        if(n.getID() == 517) {
            if(cID == -1) {
                ...
                case -1: // Two after completion dialogues (first dialogue with
                    ...
                    if(p.getQuestStage(Plugin.SHILO_VILLAGE) == -1) {
                    ...
                    } else if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 1 || p.getQuestStage(Plugin.SHILO_VILLAGE) == 2) {
                        /*
                    ...
                    } else if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 3 || p.getQuestStage(Plugin.SHILO_VILLAGE) == 4 || p.getQuestStage(Plugin.SHILO_VILLAGE) == 5) {
                        playerTalk(p, n, "Greetings...");
                        ...
                        int menu;
                        if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 4 || p.getQuestStage(Plugin.SHILO_VILLAGE) == 5) {
                            menu = showMenu(p, n,
                        ...
                        if(menu == 0) {
                            ...
                            npcTalk(p, n, "If you have found the temple, you should search it",
                                    ...
                                    "Rashiliyia.");
                            trufitisChat(p, n, Trufitus.SHOW_ME_TEMPLE_ITEMS);
                        } else if(menu == 2 && (p.getQuestStage(Plugin.SHILO_VILLAGE) == 4 || p.getQuestStage(Plugin.SHILO_VILLAGE) == 5)) {
                    ...
                    } else if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 6) {
                        playerTalk(p, n, "Greetings...");
                    ...
                    } else if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 7) {
                    ...
                    else if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 8) {
        ...
        case Trufitus.AH_ZA_RHOON:
            ...
            if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 1) {
                p.updateQuestStage(Plugin.SHILO_VILLAGE, 2);
            }
        ...
        case Trufitus.KEYS_AND_KIN:
            ...
            if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 4) {
                p.updateQuestStage(Plugin.SHILO_VILLAGE, 5);
            }
            break;
        }
    ...
    public void onObjectAction(GameObject obj, String command, Player p) {
        if (isObject(obj, QuestObjects.Snake_Jungle_Vine)) {
            if (!atQuestStage(p, this, 1) && p.getQuestStage(Plugin.LEGENDS_QUEST) == 0) {
                p.message("Yep, it looks like a vine...");
                return;
            }
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) >= 1 && p.getQuestStage(Plugin.LEGENDS_QUEST) <= 6) {
                p.message("Yep, it looks like a vine...");
                return;
            }
            ...
            if (p.getQuestStage(this) < 1 && p.getQuestStage(Plugin.LEGENDS_QUEST) == 0) {
