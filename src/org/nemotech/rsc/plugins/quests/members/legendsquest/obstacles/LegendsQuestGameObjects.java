...
import org.nemotech.rsc.plugins.quests.members.legendsquest.npcs.LegendsQuestEchnedZekin;
...
import org.nemotech.rsc.plugins.quests.members.legendsquest.npcs.LegendsQuestNezikchened;
...
public class LegendsQuestGameObjects implements ObjectActionListener, ObjectActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener {
    ...
    public void onObjectAction(GameObject obj, String command, Player p) {
        if(obj.getID() == ECHNED_ZEKIN_ROCK) {
            ...
            if(echned == null) {
                ...
                echned = spawnNpcWithRadius(LegendsQuestEchnedZekin.ECHNED_ZEKIN, p.getX(), p.getY(), 0, 60000 * 3);
                if(echned != null) {
        ...
        if(obj.getID() == WOODEN_BEAM + 1) {
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) >= 9 || p.getQuestStage(Plugin.LEGENDS_QUEST) == -1) {
            ...
            if(command.equalsIgnoreCase("look")) {
                ...
                if(menu == 0) {
                    ...
                    if(Formulae.failCalculation(p, STRENGTH, 50)) {
                        ...
                        p.setCurStat(STRENGTH, p.getCurStat(STRENGTH) - 1);
                    }
                } else if(menu == 1) {
        ...
        if(inArray(obj.getID(), SMASH_BOULDERS)) {
            if (hasItem(p, Mining.getAxe(p))) {
                ...
                if(Formulae.failCalculation(p, MINING, 50)) {
                    ...
                    p.setCurStat(MINING, p.getCurStat(MINING) - 1);
                }
            } else {
                ...
                if(p.getY() >= 3703) {
                    ...
                    if(hasItem(p, 714)) {
                        ...
                        if(Thieving.succeedPickLockThieving(p, 50)) {
                            ...
                            p.incExp(THIEVING, (int) 25.0, true);
                            replaceObjectDelayed(obj, 2000, 497);
                            p.teleport(441, 3703);
                        } else {
        ...
        if(obj.getID() == SHALLOW_WATER) {
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 8 && p.getY() >= 3723 && p.getY() <= 3740) {
                p.message("A magical looking pool.");
                return;
            }
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) >= 8 || p.getQuestStage(Plugin.LEGENDS_QUEST) == -1) {
                p.message("A disgusting sess pit of filth and stench...");
                return;
            }
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 5 || p.getQuestStage(Plugin.LEGENDS_QUEST) == 6 || p.getQuestStage(Plugin.LEGENDS_QUEST) == 7) {
                p.message("A disgusting sess pit of filth and stench...");
                return;
            }
        ...
        if(obj.getID() == ROCK) {
            if(p.getCache().hasKey("legends_cavern") || p.getQuestStage(Plugin.LEGENDS_QUEST) >= 2 || p.getQuestStage(Plugin.LEGENDS_QUEST) == -1) {
                ...
                if(menu == 0) {
                    ...
                    if(Formulae.failCalculation(p, AGILITY, 50)) {
                        ...
                        if(p.getCache().hasKey("legends_cavern")) {
                            ...
                            p.updateQuestStage(Plugin.LEGENDS_QUEST, 2);
                        }
                    } else {
        ...
        if(obj.getID() == TOTEM_POLE) { // BLACK
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) >= 10 || p.getQuestStage(Plugin.LEGENDS_QUEST) == -1) {
            ...
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 9) {
        ...
        if(obj.getID() == LEGENDS_CUPBOARD) {
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) >= 1 || p.getQuestStage(Plugin.LEGENDS_QUEST) == -1) {
                if(hasItem(p, 1172)) {
        ...
        if(obj.getID() == CRAFTED_TOTEM_POLE) {
            if(obj.getOwner().equals(p.getUsername())) {
                ...
                p.setCurStat(STRENGTH, (int) (p.getCurStat(STRENGTH) * 0.9));
            } else {
    ...
    @Override
    public void onInvUseOnObject(final GameObject obj, InvItem item, Player p) {
        if(obj.getID() == TOTEM_POLE && item.getID() == 1183) {
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) >= 10 || p.getQuestStage(Plugin.LEGENDS_QUEST) == -1) {
                replaceObjectDelayed(obj, 10000, 1170);
                message(p, 1300, "This totem pole is truly awe inspiring.",
        ...
        if(obj.getID() == TRIMMED_YOMMI_TREE && item.getID() == 405) {
            if(obj.getOwner().equals(p.getUsername())) {
                final int objectX = obj.getX();
                final int objectY = obj.getY();
                ...
                World.getWorld().getDelayedEventHandler().add(new SingleEvent(null, 60000) {
                    public void action() {
                        GameObject whatObject = RegionManager.getRegion(Point.getLocation(objectX, objectY)).getGameObject(Point.getLocation(objectX, objectY));
        ...
        if(obj.getID() == YOMMI_TREE && item.getID() == 1267) {
            ...
            World.getWorld().getDelayedEventHandler().add(new SingleEvent(null, 15000) {
                public void action() {
                    GameObject whatObject = RegionManager.getRegion(Point.getLocation(objectX, objectY)).getGameObject(Point.getLocation(objectX, objectY));
                    if(whatObject != null && whatObject.getID() == GROWN_YOMMI_TREE) {
                        ...
                        if(owner.isLoggedIn()) {
                            owner.message("The Yommi tree is past it's prime and dies .");
                        }
        ...
        if(obj.getID() == FERTILE_EARTH && item.getID() == LegendsQuestInvAction.GERMINATED_YOMMI_TREE_SEED) {
            ...
            if(getCurrentLevel(p, WOODCUTTING) < 50) {
                ...
                World.getWorld().getDelayedEventHandler().add(new SingleEvent(null, 15000) {
                    public void action() {
                        ...
                        if(whatObject != null && whatObject.getID() == YOMMI_TREE) {
                            World.getWorld().registerGameObject(new GameObject(obj.getLocation(), DEAD_YOMMI_TREE, obj.getDirection(), obj.getType(), owner.getUsername()));
                            if(owner.isLoggedIn()) {
                                owner.message("The Sapling dies.");
                            }
                            delayedSpawnObject(obj.getLoc(), 60000);
                        }
                    }
        ...
        if(obj.getID() == CARVED_ROCK) {
            ...
            case CarvedRockItems.RED_TOPAZ:
                ...
                if(alreadyAttached) {
                    ...
                    createGroundItemDelayedRemove(new Item(item.getID(), obj.getX(), obj.getY(), 1, p), 5000);
        ...
        if(item.getID() == 1249 && obj.getID() == SHALLOW_WATER) {
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 5 || p.getQuestStage(Plugin.LEGENDS_QUEST) == 6 || p.getQuestStage(Plugin.LEGENDS_QUEST) == 7) {
                ...
                if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 5) {
                    p.updateQuestStage(Plugin.LEGENDS_QUEST, 6);
                }
                return;
            }
            int emptyID = -1;
            ...
            if (emptyID != -1) {
                ...
                if(emptyID == 1188) {
                    ...
                    message(p, 1300,"You put some water in the " + EntityManager.getItem(emptyID).getName().toLowerCase() + ".");
    ...
    private void replaceTotemPole(Player p, GameObject obj) {
        if(hasItem(p, 1183)) {
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 9) {
                p.updateQuestStage(Plugin.LEGENDS_QUEST, 10);
            }
            replaceObjectDelayed(obj, 10000, 1170);
            removeItem(p, 1183, 1);
