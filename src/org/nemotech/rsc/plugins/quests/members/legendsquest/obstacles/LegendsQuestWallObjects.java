...
import org.nemotech.rsc.plugins.quests.members.legendsquest.npcs.LegendsQuestUngadulu;
import org.nemotech.rsc.plugins.quests.members.shilovillage.ShiloVillageUtils;
...
public class LegendsQuestWallObjects implements WallObjectActionListener, WallObjectActionExecutiveListener, InvUseOnWallObjectListener, InvUseOnWallObjectExecutiveListener {
    ...
    public void onWallObjectAction(GameObject obj, Integer click, Player p) {
        if(obj.getID() == ANCIENT_WALL) {
            if(click == USE) {
                if((p.getCache().hasKey("ancient_wall_runes") && p.getCache().getInt("ancient_wall_runes") == 5) || p.getQuestStage(Plugin.LEGENDS_QUEST) == -1) {
        ...
        if(obj.getID() == FLAME_WALL) {
            if(hasItem(p, 1250)) {
                ...
                if(click == TOUCH) {
                    ...
                    if(Util.random(0, 9) <= 3) {
                        message(p, 1300, "The heat is so intense that it burns you.");
                        p.damage((int) Math.ceil((double) p.getCurStat(HITS) / 10 + 1));
