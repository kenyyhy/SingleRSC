...
public class LegendsQuestSirRadimusErkle implements QuestInterface, TalkToNpcListener, TalkToNpcExecutiveListener, InvUseOnNpcListener, InvUseOnNpcExecutiveListener {
    ...
    private void skillReward(Player p, NPC n, int skill) {
        p.incQuestExp(skill, (p.getCurStat(skill) + 1) * 150);
        updateRewardClaimCount(p);
        p.message("You receive some training and increase experience to your " + Formulae.STAT_NAMES[skill] + ".");
        if(getRewardClaimCount(p) == 0) {
            npcTalk(p, n, "Right, that's all the training I can offer.! ",
                    ...
                    "Do feel free to explore the rest of the building.");
            p.sendQuestComplete(Plugin.LEGENDS_QUEST);
        } else {
            npcTalk(p, n, "You can choose " + getRewardClaimCount(p) + " areas to increase your abilities in.");
            radimusInGuildDialogue(p, n, RadimusInGuild.SKILL_MENU_ONE);
        }
        ...
        if(n.getID() == SIR_RADIMUS_ERKLE_HOUSE) {
            ...
            case RadimusInHouse.WHATS_INVOLVED:
                ...
                if(questMenu == 0) { 
                    ...
                    p.updateQuestStage(Plugin.LEGENDS_QUEST, 1);
                } else if(questMenu == 1) {
    ...
    @Override
    public boolean blockInvUseOnNpc(Player p, NPC n, InvItem item) {
    ...
    public void onInvUseOnNpc(Player p, NPC n, InvItem item) {
        if(n.getID() == SIR_RADIMUS_ERKLE_HOUSE && item.getID() == 1265) { // the glided totem.
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 11) {
                npcTalk(p, n, "Go through to the main Legends Guild and I will join you.");
                return;
            }
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 10) {
                ...
                if(!hasItem(p, 1233)) {
                    ...
                    p.updateQuestStage(Plugin.LEGENDS_QUEST, 11);
                }
            } else {
