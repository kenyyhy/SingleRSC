...
public class LegendsQuestNezikchened implements PlayerMageNpcListener, PlayerMageNpcExecutiveListener, /*PlayerNpcRunListener, PlayerNpcRunExecutiveListener,*/ PlayerKilledNpcListener, PlayerKilledNpcExecutiveListener, PlayerRangeNpcListener, PlayerRangeNpcExecutiveListener, PlayerAttackNpcExecutiveListener {
    ...
    public void onPlayerKilledNpc(Player p, NPC n) {
        if(n.getID() == NEZIKCHENED) {
            ...
            case 3: // FIRST FIGHT.
                p.setBusy(true);
                p.informOfNPCMessage(new ChatMessage(n, "Ha ha ha...I shall return for you when the time is right.", p));
                sleep(1900);
                ...
                if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 3) {
                    p.updateQuestStage(Plugin.LEGENDS_QUEST, 4);
                }
                p.setBusy(false);
            ...
            case 7:
                p.setBusy(true);
                if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 7) {
                    p.updateQuestStage(Plugin.LEGENDS_QUEST, 8);
                }
            ...
            case 8:
                if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 8) {
                    p.updateQuestStage(Plugin.LEGENDS_QUEST, 9);
                }
                if(n != null) {
                    n.remove();
                }
    ...
    public boolean blockPlayerAttackNpc(Player p, NPC n) {
        if (n.getID() == NEZIKCHENED) {
            if ((n.getAttribute("spawnedFor", null) != null && !n.getAttribute("spawnedFor").equals(p)) || p.getQuestStage(Plugin.LEGENDS_QUEST) != 7 || p.getQuestStage(Plugin.LEGENDS_QUEST) != 3) {
