...
public class LegendsQuestInvAction implements InvActionListener, InvActionExecutiveListener, InvUseOnItemListener, InvUseOnItemExecutiveListener {
    ...
    public void onInvAction(InvItem item, Player p) {
        ...
        if(item.getID() == HOLY_FORCE_SPELL) {
            ...
            if(n != null && p.getQuestStage(Plugin.LEGENDS_QUEST) == 7) {
                ...
                if(second_nezikchened != null) {
                    ...
                    second_nezikchened.setWeakend(true);
                    if(p.getCache().hasKey("ran_from_2nd_nezi")) {
                        p.informOfNPCMessage(new ChatMessage(second_nezikchened, "So you have returned and I am prepared for you now!", p));
                    } else {
                        p.informOfNPCMessage(new ChatMessage(second_nezikchened, "Now I am revealed to you Vacu, so shall ye perish.", p));
                    }
                    second_nezikchened.startCombat(p);
                    int randomMessage = Util.random(0, 3);
                    if(randomMessage == 0) {
        ...
        if(item.getID() == BOOKING_OF_BINDING) {
            ...
            if(page == 0) {
                ...
                p.getSender().sendAlert("Arcana...% % Use holy water to determine possesion, slight changes in appearance may be percieved when doused.% % Legendary Silverlight will help to defeat any demon by weakening it.% % Be wary of any demon, it may have special forms of attack. % % Use an Octagram shape to confine unearthly creatures of the underworld - the perfect geometry confuses them.", true);
            } else if(page == 1) {
                message(p, 1300, "You read the section entitled Instructo...");
                p.getSender().sendAlert("Instructo...% % To make Holy water enchant small vials to contain the magic water.% % See later chapters for enchantment. Place sacred water into vial and equip as any other missile.", true);
            } else if(page == 2) {
                message(p, 1300, "You read the section entitled Defeati...");
                p.getSender().sendAlert("Defeati...% %. Hold the book of binding open to the possesed letting the goodlight fall on them completely. Be prepared for as soon as the beast is released it will strike and strike hard.", true);
            } else if(page == 3) {
        ...
        if(inArray(item.getID(), SCRIBBLED_NOTES, SCRAWLED_NOTES, SCRATCHED_NOTES)) {
            ...
            if(item.getID() == SCRIBBLED_NOTES) {
                p.getSender().sendAlert("Daily notes of Ungadulu...% % Day 1...% % I have prepared the incantations and will invoke the spirits of my ancestors and pay them hommage. Though I feel a strange presence in these caves, it is with the heart of the lion that I fight my fears and mark the magical pentagram. % % Day 2... % % What have I done? My spirit is overthrown by a feeling of fear and evil, I am not myself these days and feel helpless and weak. % % From my teachings...                                                               ", true);
            } else if(item.getID() == SCRAWLED_NOTES) {
                p.getSender().sendAlert("I fear that the spirit of an ancient one resides within me and uses me...I am too weak to cast the curse myself and fight the beast within.% % Day 3....% %...my last hope is that someone will read this and aid me...I am undone and I fear....", true);
            } else if(item.getID() == SCRATCHED_NOTES) {
                p.getSender().sendAlert("Day 4 ...% % These days come so fleetingly, I have no idea how long I have been here now...% % Day 5... % %A wizened charm will release me, but never magic that would would harm...", true);
            }
        }
        ...
        if(item.getID() == SHAMANS_TOME) {
            ...
            p.getSender().sendAlert("% % ...scattered are my hopes that I will ever be released from this flaming Octagram, it is the only thing which will contain this beast within.% %Although it's grip over me is weakened with magic, it is hopeless to know if a saviour would guess this. % % I am doomed...", true);
        }
    }
    ...
    public void onInvUseOnItem(Player p, InvItem item1, InvItem item2) {
        if(item1.getID() == YOMMI_TREE_SEED && item2.getID() == 1267 || item1.getID() == 1267 && item2.getID() == YOMMI_TREE_SEED) {
            ...
            if(p.getQuestStage(Plugin.LEGENDS_QUEST) == 4) {
                setQuestStage(p, Plugin.LEGENDS_QUEST, 5);
            }
        }
    }
}
