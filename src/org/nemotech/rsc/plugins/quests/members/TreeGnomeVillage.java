...
PlayerKilledNpcExecutiveListener {
    ...
    public int getQuestID() {
        return Plugin.TREE_GNOME_VILLAGE;
    }
    ...
    public void handleReward(Player p) {
        ...
        p.incQuestExp(0, 6950 + 75 * p.getMaxStat(0));
        addItem(p, 744, 1);
        ...
        if (n.getID() == 400) {
            ...
            if (p.getQuestStage(getQuestID()) == 6) {
                if (hasItem(p, 741)) {
                    ...
                    p.sendQuestComplete(Plugin.TREE_GNOME_VILLAGE);
                } else {
    ...
    public void onWallObjectAction(GameObject obj, Integer click, Player p) {
        ...
        if(obj.getID() == 101 && obj.getX() == 540 && obj.getY() == 445) {
            ...
            if (p.getQuestStage(Plugin.FISHING_CONTEST) >= 1 || p.getQuestStage(Plugin.PLAGUE_CITY) >= 1) {
                p.message(
