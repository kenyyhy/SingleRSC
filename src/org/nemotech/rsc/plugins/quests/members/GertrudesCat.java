...
ObjectActionExecutiveListener, DropListener, DropExecutiveListener {
    ...
    public void handleReward(final Player p) {
        p.incQuestExp(COOKING, p.getMaxStat(COOKING) * 45 + 175);
        p.incQuestPoints(1);
    ...
    public void onTalkToNpc(final Player p, final NPC n) {
        ...
        if (n.getID() == 714) {
            ...
            case 3:
                ...
                addItem(p, 332, 1);
                addItem(p, 346, 1);
                p.sendQuestComplete(Plugin.GERTRUDES_CAT);
                break;
            case -1:
                ...
                if (hasItem(p, 1096)) {
                    ...
                    if (menu == 0) {
                        ...
                        if (p.getInventory().countId(10) >= 100) {
                            ...
                            p.getInventory().add(new InvItem(1096));
                        } else {
                            playerTalk(p, n,
    ...
    public void onWallObjectAction(GameObject obj, Integer click, Player p) {
        if (obj.getID() == 199 && obj.getY() == 438) {
            if (p.getQuestStage(Plugin.GERTRUDES_CAT) >= 2
                    || p.getQuestStage(Plugin.GERTRUDES_CAT) == -1) {
    ...
    @Override
    public boolean blockInvUseOnGroundItem(InvItem myItem, Item item,
            Player player) {
    ...
    @Override
    public void onInvUseOnGroundItem(InvItem myItem, Item item, Player p) {
        if (myItem.getID() == 22 && item.getID() == 1093) {
