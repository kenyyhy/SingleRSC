package org.nemotech.rsc.plugins.quests.members.shilovillage;
...
public class ShiloVillageObjects implements ObjectActionListener, ObjectActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener {
    ...
    public void onObjectAction(GameObject obj, String command, Player p) {
        ...
        if(obj.getID() == HILLSIDE_ENTRANCE) {
            if(command.equalsIgnoreCase("Open")) {
                ...
                if(!p.getCache().hasKey("can_chisel_bone") && p.getQuestStage(Plugin.SHILO_VILLAGE) == 7) {
        ...
        if(obj.getID() == SEARCH_TREE_FOR_ENTRANCE) {
            ...
            GameObject ENTRANCE = new GameObject(Point.getLocation(350, 782), HILLSIDE_ENTRANCE, 2, 0);
            registerObject(ENTRANCE);
            delayedSpawnObject(new GameObject(Point.getLocation(350, 782), 398, 2, 0).getLoc(), 15000);
        }
        if(obj.getID() == TOMB_DOLMEN_HANDHOLDS) {
        ...
        if(obj.getID() == WELL_STACKED_ROCKS) {
            if(command.equalsIgnoreCase("Investigate")) {
                ...
                if(p.getQuestStage(Plugin.SHILO_VILLAGE) == -1) {
                ...
                } else if(p.getQuestStage(Plugin.SHILO_VILLAGE) >= 5) {
                    ...
                    if(menu == 0) {
                        ...
                        if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 5) {
                            p.updateQuestStage(Plugin.SHILO_VILLAGE, 6);
                        }
                    } else if(menu == 1) {
        ...
        if(obj.getID() == ROTTEN_GALLOWS) {
            if(command.equalsIgnoreCase("Look")) {
                ...
                if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 3) {
                    p.message("His hands have been tied behind his back.");
                }
            } else if(command.equalsIgnoreCase("Search")) {
                message(p, "You search the gallows.");
                if(hasItem(p, ZADIMUS_CORPSE) || p.getQuestStage(Plugin.SHILO_VILLAGE) >= 4) {
        ...
        if(obj.getID() == SMASHED_TABLE) {
            ...
            if(command.equalsIgnoreCase("Examine")) {
                ...
                if(sub == 0) {
                    ...
                    GameObject RAFT_ONE = new GameObject(Point.getLocation(353, 3669), 698, 0, 0);
                    ...
                    GameObject RAFT_TWO = new GameObject(Point.getLocation(357, 3673), 698, 0, 0);
                    ...
                    GameObject RAFT_THREE = new GameObject(Point.getLocation(356, 3678), 698, 0, 0);
                    registerObject(RAFT_THREE);
                    p.teleport(356, 3678);
                    sleep(1000);
                    removeObject(RAFT_THREE);

                    GameObject RAFT_FOUR = new GameObject(Point.getLocation(353, 3682), 698, 0, 0);
                    ...
                    GameObject RAFT_FIVE = new GameObject(Point.getLocation(349, 3685), 698, 0, 0);
                    registerObject(RAFT_FIVE);
                    p.teleport(349, 3685);
                    sleep(500);
                    removeObject(RAFT_FIVE);

                    GameObject RAFT_SIX = new GameObject(Point.getLocation(345, 3686), 698, 0, 0);
                    ...
                    GameObject RAFT_FINAL = new GameObject(Point.getLocation(341, 3686), 698, 0, 0);
        ...
        if(obj.getID() == BUMPY_DIRT) {
            ...
            } else if(p.getQuestStage(Plugin.SHILO_VILLAGE) >= 2) {
    ...
    public void onInvUseOnObject(GameObject obj, InvItem item, Player p) {
        ...
        if(obj.getID() == HILLSIDE_ENTRANCE && item.getID() == 835) {
            ...
            if(p.getQuestStage(Plugin.SHILO_VILLAGE) == 7) {
                p.updateQuestStage(Plugin.SHILO_VILLAGE, 8);
            }
        }
        if(obj.getID() == HILLSIDE_ENTRANCE && item.getID() == 974) {
