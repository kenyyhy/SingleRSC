...
public class ShiloVillageUtils implements DropListener, DropExecutiveListener, InvActionListener, InvActionExecutiveListener, InvUseOnItemListener, InvUseOnItemExecutiveListener, PickupListener, PickupExecutiveListener {
    ...
    public static void BUMPY_DIRT_HOLDER(Player p) {
        ...
        if(menu == 0) {
            ...
            p.updateQuestStage(Plugin.SHILO_VILLAGE, 3);
        } else if(menu == 1) {
    ...
    private void dropZadimusCorpse(Player p) {
        ...
        if(p.getLocation().inBounds(445, 749, 449, 753)) {
            ...
            if(zadimus != null) {
                ...
                setQuestStage(p, Plugin.SHILO_VILLAGE, 4);
            }
        } else {
    ...
    public void onInvAction(InvItem item, Player p) {
        ...
        if(item.getID() == 972) { // activate crystal
            ...
            if(menu == 0) {
                ...
                if(objectX - p.getX() <= 5 && objectX - p.getX() >= -5) {
                    ...
                    p.setCurStat(PRAYER, p.getCurStat(PRAYER) - 1);
                    ...
                    p.setCurStat(PRAYER, p.getCurStat(PRAYER) - 2);
                }
            } else if(menu == 1) {
        ...
        if(item.getID() == 961) { // read tomb notes
            ...
            if(menu == 0) {
                p.getSender().sendAlert("...and rest like your mother who is silent in the peace of her "
                ...
                p.getSender().sendAlert("...Rashiliyia did so love objects of beauty. Her tomb was "
                        + "adnorned with crystals that glowed brightly when near to each other.", false);
            } else if(menu == 2) {
                p.getSender().sendAlert("...the sphere is activated when power of a spiritual nature is "
        ...
        if(item.getID() == ShiloVillageObjects.CRUMPLED_SCROLL) {
            ...
            int menu = showMenu(p,
                    ...
                    "No thanks.");
            if(menu == 0) {
                p.getSender().sendAlert("Rashiliyia's rage went unchecked.% %"
        ...
        if(item.getID() == ShiloVillageObjects.TATTERED_SCROLL) {
            ...
            int menu = showMenu(p,
                    ...
                    "No thanks.");
            if(menu == 0) {
                p.getSender().sendAlert("Bervirius, song of King Danthalas, was killed in battle.% %"
    ...
    public void onInvUseOnItem(Player p, InvItem item1, InvItem item2) {
        ...
        if(item1.getID() == 167 && item2.getID() == 974 || item1.getID() == 974 && item2.getID() == 167) {
            ...
            if(p.getCache().hasKey("can_chisel_bone")) {
                ...
                p.incExp(CRAFTING, (int) 8.0, true);
            } else {
        ...
        if(item1.getID() == 167 && item2.getID() == 973 || item1.getID() == 973 && item2.getID() == 167) {
            ...
            p.incExp(CRAFTING, (int) 9.0, true);
