...
public class DigsiteDigAreas implements ObjectActionListener, ObjectActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener, InvActionListener, InvActionExecutiveListener {
    ...
    private void doSpade(Player p, InvItem item, GameObject obj) {
    ...
    private void rockPickOnSite(Player p, InvItem item, GameObject obj) {
        if(item.getID() == ROCK_PICK && inArray(obj.getID(), SOIL)) {
            ...
            if(p.getQuestStage(Plugin.DIGSITE) < 4 && getLevel2Digsite(p)) {
                NPC workman = spawnNpc(WORKMAN, p.getX(), p.getY(), 30000);
                if(workman != null) {
            ...
            if(p.getQuestStage(Plugin.DIGSITE) >= 4 && getLevel2Digsite(p)) {
                ...
                showBubble(p, new InvItem(ROCK_PICK));
                p.incExp(MINING, (int) 17.5, true);
                message(p, "You dig through the earth");
                sleep(1500);
                int randomize = Util.random(0, (DIGSITE_LEVEL2_ITEMS.length - 1));
    ...
    private void trowelOnSite(Player p, InvItem item, GameObject obj) {
        if(item.getID() == TROWEL && inArray(obj.getID(), SOIL)) {
            if(getTrainingAreas(p)) {
                showBubble(p, new InvItem(TROWEL));
                p.incExp(MINING, (int) 12.5, true);
                message(p, "You dig with the trowel...");
                sleep(1500);
                int randomize = Util.random(0, (TRAINING_AREA_ITEMS.length - 1));
            ...
            if(getLevel1Digsite(p)) {
                ...
                showBubble(p, new InvItem(TROWEL));
                p.incExp(MINING, (int) 15.0, true);
                message(p, "You dig through the earth");
                sleep(1500);
                int randomize = Util.random(0, (DIGSITE_LEVEL1_ITEMS.length - 1));
            ...
            if(getLevel3Digsite(p)) {
                ...
                if(p.getQuestStage(Plugin.DIGSITE) < 5) {
                    NPC workman = spawnNpc(WORKMAN, p.getX(), p.getY(), 30000);
                    if(workman != null) {
                ...
                showBubble(p, new InvItem(TROWEL));
                p.incExp(MINING, (int) 20.0, true);
                message(p, "You dig through the earth");
                sleep(1500);
                int randomize = Util.random(0, (DIGSITE_LEVEL3_ITEMS.length - 1));
                int selectedItem = DIGSITE_LEVEL3_ITEMS[randomize];
                doDigsiteItemMessages(p, selectedItem);
                if(selectedItem != -1) {
                    if(selectedItem == 10) {
                        addItem(p, 10, (Util.random(0, 1) == 1 ? 5 : 10));
                    } else {
                        addItem(p, selectedItem, 1);
                    }
                }
    ...
    public void onObjectAction(GameObject obj, String command, Player p) {
        if(inArray(obj.getID(), SOIL)) {
            p.message("You examine the patch of soil");
