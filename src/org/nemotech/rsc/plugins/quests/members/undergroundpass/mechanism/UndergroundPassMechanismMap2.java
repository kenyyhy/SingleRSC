...
import org.nemotech.rsc.plugins.quests.members.undergroundpass.obstacles.UndergroundPassObstaclesMap2;
...
public class UndergroundPassMechanismMap2 implements InvUseOnObjectListener, InvUseOnObjectExecutiveListener {
    ...
    public void onInvUseOnObject(GameObject obj, InvItem item, Player p) {
        ...
        if(obj.getID() == BOULDER && item.getID() == RAILING) {
            ...
            if(p.getQuestStage(Plugin.UNDERGROUND_PASS) == 3) {
                p.updateQuestStage(Plugin.UNDERGROUND_PASS, 4);
            }
        }
        if(obj.getID() == UndergroundPassObstaclesMap2.FLAMES_OF_ZAMORAK && inArray(item.getID(), ITEMS_TO_FLAMES)) {
            message(p, "you throw the " + item.getDef().getName().toLowerCase() + " into the flames");
            if(!atQuestStage(p, Plugin.UNDERGROUND_PASS, 7) || !atQuestStage(p, Plugin.UNDERGROUND_PASS, -1)) {
