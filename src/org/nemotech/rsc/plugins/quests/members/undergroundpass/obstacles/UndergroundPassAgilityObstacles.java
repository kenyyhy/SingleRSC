...
public class UndergroundPassAgilityObstacles implements ObjectActionListener, ObjectActionExecutiveListener {
    ...
    public void onObjectAction(GameObject obj, String command, Player p) {
        if(inArray(obj.getID(), LEDGES)) {
            ...
            if(succeed(p, 1)) {
                ...
                case 862:
                    ...
                    break;
                case 864:
                    p.teleport((p.getQuestStage(Plugin.UNDERGROUND_PASS) >= 4) || (p.getQuestStage(Plugin.UNDERGROUND_PASS) == -1)  ? 751 : 734, 3496);
                    break;
                case 863:
                    p.teleport(763, 3442);
                    break;
                case 872:
                    p.teleport((p.getQuestStage(Plugin.UNDERGROUND_PASS) >= 4) || (p.getQuestStage(Plugin.UNDERGROUND_PASS) == -1) ? 765 : 748, 3497);
                    break;
                case 865:
                    p.teleport(728, 3499);
                    break;
                case 866:
                    p.teleport((p.getQuestStage(Plugin.UNDERGROUND_PASS) >= 4) || (p.getQuestStage(Plugin.UNDERGROUND_PASS) == -1) ? 755 : 738, 3501);
                    break;
                }
        ...
        if(obj.getID() == NORTH_STONE_STEP) {
            if(p.getQuestStage(Plugin.UNDERGROUND_PASS) == 4) {
        ...
        if(inArray(obj.getID(), STONE_REMAINING_BRIDGES) || inArray(obj.getID(), STONE_JUMP_BRIDGES)) {
            ...
            World.getWorld().getDelayedEventHandler().add(new UndergroundPassMessageEvent(p, Util.random(3000, 15000)));
        }
    }
    ...
    private void failBlackAreaObstacle(Player p, GameObject obj) {
        ...
        if(p.getQuestStage(Plugin.UNDERGROUND_PASS) == 4) {
            p.updateQuestStage(Plugin.UNDERGROUND_PASS, 5);
