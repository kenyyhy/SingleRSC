...
import static org.nemotech.rsc.plugins.Plugin.THIEVING;
    ...
    public void onObjectAction(GameObject obj, String command, Player p) {
        ...
        if(obj.getID() == LEDGE) {
            if(command.equalsIgnoreCase("climb up")) {
                ...
                if(Util.getRandom().nextInt(10) <= 1) {
        ...
        if(obj.getID() == CAGE_REMAINS) {
            if(p.getQuestStage(Plugin.UNDERGROUND_PASS) >= 5 || p.getQuestStage(Plugin.UNDERGROUND_PASS) == -1) {
        ...
        if(obj.getID() == GATE_OF_IBAN) {
            ...
            if((p.getCache().hasKey("flames_of_zamorak1") && p.getCache().hasKey("flames_of_zamorak2") && (p.getCache().hasKey("flames_of_zamorak3") && p.getCache().getInt("flames_of_zamorak3") >= 2)) || atQuestStage(p, Plugin.UNDERGROUND_PASS, 7) || atQuestStage(p, Plugin.UNDERGROUND_PASS, -1)) {
        ...
        if(obj.getID() == FLAMES_OF_ZAMORAK) {
            ...
            p.message("it reads...");
            p.getSender().sendAlert("@dre@While I sense the soft beating of a good heart I will not open% %@dre@Feed me three crests of the blessed warriors, and the%@dre@creatures remains% %@dre@Throw them to me as an offering, a gift of hatred, a token% %@dre@Then finally rejoice as all goodness dies in my flames", true); 
        }
    }
    ...
    public void onWallObjectAction(GameObject obj, Integer click, Player p) {
        if(inArray(obj.getID(), RAILINGS)) {
            if(click == 0) {
                ...
                p.incExp(THIEVING, (int) 3.75, true);
                sleep(1600);
