...
public class UndergroundPassOrbs implements ObjectActionListener, ObjectActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener,PickupExecutiveListener {
    ...
    public void onObjectAction(GameObject obj, String cmd, Player p) {
        ...
        if(inArray(obj.getID(), WEST_PASSAGE)) {
            if(cmd.equalsIgnoreCase("clear")) {
                ...
                if(menu == 0) {
                    ...
                    if(Util.getRandom().nextInt(20) <= 2) {
        ...
        if(obj.getID() == SOUTH_WEST_PASSAGE_CLIMB_UP) {
            ...
            if(Util.getRandom().nextInt(10) <= 2) { // fail
    ...
    public void onInvUseOnObject(GameObject obj, InvItem item, Player player) {
        if(item.getID() == PLANK && (obj.getID() == NORTH_PASSAGE[0] || obj.getID() == NORTH_PASSAGE[2])) {
            ...
            GameObject object = new GameObject(Point.getLocation(728, 3435), 827, 0, 0);
        ...
        if(inArray(item.getID(), ORBS) && obj.getID() == FURNACE) {
            ...
            if(!atQuestStage(player, Plugin.UNDERGROUND_PASS, 7) || !atQuestStage(player, Plugin.UNDERGROUND_PASS, -1)) {
        ...
        private void fallBack(Player p, GameObject old) {
            if(old.getID() == WEST_PASSAGE[0]) {
                ...
                p.teleport(738, 3446); 
                sleep(2000);
                damageOfTrap(p, old, new GameObject(Point.getLocation(739, 3446), 773, 2, 0), 820);
                p.teleport(735, 3446); 
                firstFallbackTrap(p, old);
                ...
                p.teleport(741, 3446); 
                sleep(2000);
                damageOfTrap(p, old, new GameObject(Point.getLocation(742, 3446), 773, 2, 0), 821);
                p.teleport(738, 3446); 
                sleep(2000);
                damageOfTrap(p, old, new GameObject(Point.getLocation(739, 3446), 773, 2, 0), 820);
                p.teleport(735, 3446); 
                ...
                p.teleport(744, 3446); 
                sleep(2000);
                damageOfTrap(p, old, new GameObject(Point.getLocation(745, 3446), 773, 2, 0), 822);
                p.teleport(741, 3446); 
                sleep(2000);
                damageOfTrap(p, old, new GameObject(Point.getLocation(742, 3446), 773, 2, 0), 821);
                p.teleport(738, 3446); 
                ...
                p.teleport(747, 3446); 
                sleep(2000);
                damageOfTrap(p, old, new GameObject(Point.getLocation(748, 3446), 773, 2, 0), 823);
                p.teleport(744, 3446); 
                sleep(2000);
                damageOfTrap(p, old, new GameObject(Point.getLocation(745, 3446), 773, 2, 0), 822);
                p.teleport(741, 3446); 
        ...
        private void firstFallbackTrap(Player p, GameObject old) {
            sleep(2000);
            p.message("you hear a strange mechanical sound");
            damageOfTrap(p, old, new GameObject(Point.getLocation(736, 3446), 773, 2, 0), 819);
            sleep(1600);
            p.message("You've triggered a trap");
        }
        ...
        private void damageOfTrap(Player p, GameObject obj, GameObject _new, int objectID) {
            ...
            if(_new == null) {
                ...
                playerTalk(p,null, "aaarrghhh");
                World.getWorld().registerGameObject(new GameObject(Point.getLocation(p.getX() + 1, p.getY()), objectID, 2, 0));
            }
        }
    }
