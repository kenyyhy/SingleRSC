...
public class WatchTowerDialogues implements QuestInterface, TalkToNpcListener, TalkToNpcExecutiveListener {
    ...
    public void onTalkToNpc(Player p, NPC n) {
        if(n.getID() == SKAVID_679) {
            if(p.getCache().hasKey("skavid_completed_language") || p.getQuestStage(Plugin.WATCHTOWER) == -1) {
                npcTalk(p, n, "What, you gots the crystal...");
                int lastMenu = showMenu(p, n, "But I've lost it!", "Oh okay then");
                if(lastMenu == 0) {
                    if(hasItem(p, POWERING_CRYSTAL2) || p.getQuestStage(Plugin.WATCHTOWER) == -1) {
                ...
                int randomizeChat = Util.random(0, sayChat.length - 1);
        ...
        if(inArray(n.getID(), SKAVID_696, SKAVID_707, SKAVID_708, SKAVID_709)) {
            if(n.getID() == SKAVID_696) {
                if(p.getCache().hasKey("language_ig") || p.getCache().hasKey("skavid_completed_language") || p.getQuestStage(Plugin.WATCHTOWER) == -1) {
                ...
                if(p.getCache().hasKey("language_ar") || p.getCache().hasKey("skavid_completed_language") || p.getQuestStage(Plugin.WATCHTOWER) == -1) {
                    npcTalk(p, n, "Ar cur...");
                    p.message("You have already talked to this skavid");
                    return;
                }
                ...
                if(p.getCache().hasKey("language_cur") || p.getCache().hasKey("skavid_completed_language") || p.getQuestStage(Plugin.WATCHTOWER) == -1) {
                ...
                if(p.getCache().hasKey("language_nod") || p.getCache().hasKey("skavid_completed_language") || p.getQuestStage(Plugin.WATCHTOWER) == -1) {
                    npcTalk(p, n, "Gor nod...");
                    p.message("You have already talked to this skavid");
                    return;
                }
        ...
        if(n.getID() == SKAVID_678) {
            ...
            if((p.getCache().hasKey("language_cur") 
                ...
                if(menu == 0) {
                    ...
                    p.updateQuestStage(Plugin.WATCHTOWER, 5);
                }
            }
        }
            ...
            if(cID == -1) {
                ...
                case 7:
                    ...
                    if(hasItem(p, 1053)) {
                        ...
                        removeItem(p, 1053, 1);
                        addItem(p, 1054, 1);
                        if(p.getQuestStage(Plugin.WATCHTOWER) == 7) {
                            p.updateQuestStage(Plugin.WATCHTOWER, 8);
                        }
