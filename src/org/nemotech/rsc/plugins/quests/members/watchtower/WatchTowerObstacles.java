...
import static org.nemotech.rsc.plugins.Plugin.coordModifier;
...
public class WatchTowerObstacles implements ObjectActionListener, ObjectActionExecutiveListener, WallObjectActionListener, WallObjectActionExecutiveListener {
    ...
    public void onObjectAction(GameObject obj, String command, Player p) {
        ...
        if(obj.getID() == TOWER_FIRST_FLOOR_LADDER) {
            ...
            switch(p.getQuestStage(Plugin.WATCHTOWER)) {
            case 0:
                if(t_guard != null) {
        ...
        if(inArray(obj.getID(), WRONG_BUSHES)) {
            if(p.getQuestStage(Plugin.WATCHTOWER) == 0) {
                    ...
                    if(ogre_guard != null) {
                        ...
                        if(menu == 0) {
                            ...
                            if(!hasItem(p, 10, 20)) {
                                ...
                                p.teleport(647, 799);
                                p.incExp(AGILITY, (int) 12.5, true);
        ...
        if(obj.getID() == SKAVID_HOLE) {
            if(p.getFatigue() >= 7050) {
                p.message("You are too tired to enter this tunnel");
                return;
            }
            p.message("You enter the tunnel");
            p.message("So that's how the skavids are getting into yanille!");
            p.teleport(609, 742);
            p.incExp(Plugin.AGILITY, (int) 10.0, true);
        }
    }
    ...
    public void onWallObjectAction(GameObject obj, Integer click, Player p) {
        ...
        if(obj.getID() == SOUTH_WEST_BATTLEMENT) {
            ...
            if(p.getX() <= 664) {
                ...
                } else if(p.getCache().hasKey("get_ogre_gift") || p.getQuestStage(Plugin.WATCHTOWER) == -1) {
                    if(ogre_guard != null) {
