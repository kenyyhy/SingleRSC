...
TalkToNpcExecutiveListener, ObjectActionListener, ObjectActionExecutiveListener, PickupListener, PickupExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener, PlayerMageNpcListener, PlayerMageNpcExecutiveListener, PlayerKilledNpcListener, PlayerKilledNpcExecutiveListener, PlayerAttackNpcListener, PlayerAttackNpcExecutiveListener, PlayerRangeNpcListener, PlayerRangeNpcExecutiveListener {
    ...
    public void handleReward(Player p) {
        ...
        p.incQuestExp(RANGED, (p.getMaxStat(RANGED) * 250) + 500);
        p.incQuestExp(FLETCHING, (p.getMaxStat(FLETCHING) * 250) + 500);
        ...
        if(n.getID() == WINELDA) {
            if(cID == -1) {
                if(hasItem(p, LIMPROOT, 20)) {
                    ...
                    p.teleport(557, 3290);
                    sleep(650);
                    p.getSender().sendTeleBubble(p.getX(), p.getY(), false);
                } else {
        ...
        if(n.getID() == EDGE_LUCIEN) {
            ...
            if(hasItem(p, 725)) {
                ...
                if(menu == 0) {
                    ...
                    p.sendQuestComplete(Plugin.TEMPLE_OF_IKOV);
                    p.updateQuestStage(this, -2);
    ...
    public void onPlayerKilledNpc(Player p, NPC n) {
        ...
        if(n.getID() == EDGE_LUCIEN) {
            if(p.getQuestStage(this) == -1 || p.getQuestStage(this) == -2) {
                p.message("You have already completed this quest");
                n.setCurHits(n.getCurHits()); // iks this right?? TODFO
                return;
            }
            n.setCurHits(n.getCurHits()); // iks this right?? TODFO
            npcTalk(p, n, "You may have defeated me for now", "But I will be back");
            p.sendQuestComplete(Plugin.TEMPLE_OF_IKOV);
    ...
    public boolean blockPlayerRangeNpc(Player p, NPC n) {
        if(n.getID() == EDGE_LUCIEN && (p.getQuestStage(Plugin.TEMPLE_OF_IKOV) == -1 || p.getQuestStage(Plugin.TEMPLE_OF_IKOV) == -2 || !p.getInventory().wielding(726))) {
            return true;
        }
        if(n.getID() == WARRIOR_OF_LESARKUS && (p.getCache().hasKey("killedLesarkus") || p.getQuestStage(Plugin.TEMPLE_OF_IKOV) == -1 || p.getQuestStage(Plugin.TEMPLE_OF_IKOV) == -2 || !hasItem(p, 723))) {
            return true;
        }
        return false;
    }
    ...
    public void onPlayerRangeNpc(Player p, NPC n) {
        if(n.getID() == EDGE_LUCIEN) {
            if(p.getQuestStage(Plugin.TEMPLE_OF_IKOV) == -1 || p.getQuestStage(Plugin.TEMPLE_OF_IKOV) == -2) {
                p.message("You have already completed this quest");
                return;
            }
        ...
        if(n.getID() == WARRIOR_OF_LESARKUS) {
            if((p.getCache().hasKey("killedLesarkus") || p.getQuestStage(Plugin.TEMPLE_OF_IKOV) == -1 || p.getQuestStage(Plugin.TEMPLE_OF_IKOV) == -2)) {
                p.message("You have already killed the fire warrior");
                return;
            }
            if(!hasItem(p, 723)) {
