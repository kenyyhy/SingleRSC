...
public class ShieldOfArrav implements QuestInterface,InvUseOnWallObjectListener,
    ...
    public void onObjectAction(GameObject obj, String command, Player player) {
        ...
        case 67:
            ...
            break;
        case 81:
            if (player.getBank().contains(new InvItem(54))
                    || player.getInventory().contains(new InvItem(54))) {
            ...
            break;
        case 85:
            if (player.getBank().contains(new InvItem(53))
                    || player.getInventory().contains(new InvItem(53))) {
            ...
            case -1:
                if (p.getCache().hasKey("arrav_gang")) {
                    if (p.getCache().getInt("arrav_gang") == BLACK_ARM
                            && p.getQuestStage(Plugin.HEROS_QUEST) >= 1) {
                        playerTalk(p, n, "Hey");
                        npcTalk(p, n, "Hey");
                        if (hasItem(p, 585)) {
                        ...
                        if (choice2 == 0) {
                            ...
                            && (p.getQuestStage(Plugin.HEROS_QUEST) != 1 && p
                                    .getQuestStage(Plugin.HEROS_QUEST) != 2)
                            || (p.getCache().getInt("arrav_gang") == BLACK_ARM && p
                                    .getQuestStage(Plugin.HEROS_QUEST) == -1)) {
                        playerTalk(p, n, "Hey");
                        npcTalk(p, n, "Hey");
    ...
    public void onPlayerKilledNpc(Player p, NPC n) {
        if (p.getQuestStage(this) == 4) {
            World.getWorld().registerItem(
                    new Item(QuestItems.SCROLL, n.getX(), n.getY(), 1, p));
            n.killedBy(p);
        } else {
            n.killedBy(p);
        }
