...
public class MagicInstructor implements TalkToNpcExecutiveListener, TalkToNpcListener {
    ...
    public void onTalkToNpc(Player p, NPC n) {
        if(p.getExp(6) > 0 && p.getCache().hasKey("tutorial") && p.getCache().getInt("tutorial") == 80) {
            npcTalk(p, n, "Please proceed through the next door");
            return;
        }
        if(p.getExp(6) > 0 && p.getCache().hasKey("tutorial") && p.getCache().getInt("tutorial") == 78) {
        ...
        if(p.getCache().hasKey("tutorial") && p.getCache().getInt("tutorial") == 75 && p.getExp(6) <= 0) {
