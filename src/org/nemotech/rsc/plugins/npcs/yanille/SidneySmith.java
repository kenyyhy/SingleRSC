...
public class SidneySmith implements TalkToNpcListener, TalkToNpcExecutiveListener, InvUseOnNpcListener, InvUseOnNpcExecutiveListener {
    ...
    private void goodsMenuOne(Player p, NPC n) {
        ...
        if(goods == 0) {
            if(hasItem(p, PRAYER_RESTORE_POT, 5)) {
                calculateExchangeMenu(p, n, false, new InvItem(PRAYER_RESTORE_POT), new InvItem(PRAYER_CERT));
            } else {
            ...
            if(hasItem(p, SUPER_ATTACK_POT, 5)) {
                calculateExchangeMenu(p, n, false, new InvItem(SUPER_ATTACK_POT), new InvItem(SUPER_ATTACK_CERT));
            } else {
            ...
            if(hasItem(p, SUPER_DEFENSE_POT, 5)) {
                calculateExchangeMenu(p, n, false, new InvItem(SUPER_DEFENSE_POT), new InvItem(SUPER_DEFENSE_CERT));
            } else {
            ...
            if(hasItem(p, SUPER_STRENGTH_POT, 5)) {
                calculateExchangeMenu(p, n, false, new InvItem(SUPER_STRENGTH_POT), new InvItem(SUPER_STRENGTH_CERT));
            } else {
    ...
    private void goodsMenuTwo(Player p, NPC n) {
        ...
        if(goods == 0) {
            if(hasItem(p, DRAGON_BONES, 5)) {
                calculateExchangeMenu(p, n, false, new InvItem(DRAGON_BONES), new InvItem(DRAGON_BONES_CERT));
            } else {
            ...
            if(hasItem(p, LIMPWURT_ROOT, 5)) {
                calculateExchangeMenu(p, n, false, new InvItem(LIMPWURT_ROOT), new InvItem(LIMPWURT_ROOT_CERT));
            } else {
    ...
    private void certMenuOne(Player p, NPC n) {
        ...
        if(certs == 0) {
            if(hasItem(p, PRAYER_CERT)) {
                calculateExchangeMenu(p, n, true, new InvItem(PRAYER_RESTORE_POT), new InvItem(PRAYER_CERT));
            } else {
            ...
            if(hasItem(p, SUPER_ATTACK_CERT)) {
                calculateExchangeMenu(p, n, true, new InvItem(SUPER_ATTACK_POT), new InvItem(SUPER_ATTACK_CERT));
            } else {
                npcTalk(p,n, "Sorry, but you don't have any ",
            ...
            if(hasItem(p, SUPER_DEFENSE_CERT)) {
                calculateExchangeMenu(p, n, true, new InvItem(SUPER_DEFENSE_POT), new InvItem(SUPER_DEFENSE_CERT));
            } else {
                npcTalk(p,n, "Sorry, but you don't have any ",
            ...
            if(hasItem(p, SUPER_STRENGTH_CERT)) {
                calculateExchangeMenu(p, n, true, new InvItem(SUPER_STRENGTH_POT), new InvItem(SUPER_STRENGTH_CERT));
            } else {
                npcTalk(p,n, "Sorry, but you don't have any ",
    ...
    private void certMenuTwo(Player p, NPC n) {
        ...
        if(menu == 0) {
            if(hasItem(p, DRAGON_BONES_CERT)) {
                calculateExchangeMenu(p, n, true, new InvItem(DRAGON_BONES), new InvItem(DRAGON_BONES_CERT));
            } else {
                npcTalk(p,n, "Sorry, but you don't have any ",
            ...
            if(hasItem(p, LIMPWURT_ROOT_CERT)) {
                calculateExchangeMenu(p, n, true, new InvItem(LIMPWURT_ROOT), new InvItem(LIMPWURT_ROOT_CERT));
            } else {
                npcTalk(p,n, "Sorry, but you don't have any ",
    ...
    private void calculateExchangeMenu(Player p, NPC n, boolean useCertificate, InvItem i, InvItem certificate) {
            ...
            if(useCertificate) {
                ...
                if (p.getInventory().remove(certificate.getID(), mainMenu) > -1) {
                    for (int x = 0; x < itemAmount; x++) {
                        p.getInventory().add(new InvItem(i.getID(), 1));
                    }
                }
                playerTalk(p,n, "Ok thanks.");
            } else {
                ...
                p.getInventory().add(new InvItem(certificate.getID(), mainMenu));
                playerTalk(p,n, "Ok thanks.");
            }
        }
    }
    ...
    public void onInvUseOnNpc(Player player, NPC npc, InvItem item) {
        if(npc.getID() == SIDNEY_SMITH && inArray(item.getID(), PRAYER_RESTORE_POT, 
            ...
            case PRAYER_RESTORE_POT:
                calculateExchangeMenu(player, npc, false, item, new InvItem(PRAYER_CERT));
                break;
            case SUPER_ATTACK_POT:
                calculateExchangeMenu(player, npc, false, item, new InvItem(SUPER_ATTACK_CERT));
                break;
            case SUPER_STRENGTH_POT:
                calculateExchangeMenu(player, npc, false, item, new InvItem(SUPER_STRENGTH_CERT));
                break;
            case SUPER_DEFENSE_POT:
                calculateExchangeMenu(player, npc, false, item, new InvItem(SUPER_DEFENSE_CERT));
                break;
            case DRAGON_BONES:
                calculateExchangeMenu(player, npc, false, item, new InvItem(DRAGON_BONES_CERT));
                break;
            case LIMPWURT_ROOT:
                calculateExchangeMenu(player, npc, false, item, new InvItem(LIMPWURT_ROOT_CERT));
                break;
            case PRAYER_CERT:
                calculateExchangeMenu(player, npc, true, new InvItem(PRAYER_RESTORE_POT), item);
                break;
            case SUPER_ATTACK_CERT:
                calculateExchangeMenu(player, npc, true, new InvItem(SUPER_ATTACK_POT), item);
                break;
            case SUPER_STRENGTH_CERT:
                calculateExchangeMenu(player, npc, true, new InvItem(SUPER_STRENGTH_POT), item);
                break;
            case SUPER_DEFENSE_CERT:
                calculateExchangeMenu(player, npc, true, new InvItem(SUPER_DEFENSE_POT), item);
                break;
            case DRAGON_BONES_CERT:
                calculateExchangeMenu(player, npc, true, new InvItem(DRAGON_BONES), item);
                break;
            case LIMPWURT_ROOT_CERT:
                calculateExchangeMenu(player, npc, true, new InvItem(LIMPWURT_ROOT), item);
                break;
            default:
