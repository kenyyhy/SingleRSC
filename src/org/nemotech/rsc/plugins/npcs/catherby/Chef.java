...
public class Chef implements TalkToNpcExecutiveListener, TalkToNpcListener {
    ...
    public void onTalkToNpc(final Player p, final NPC n) {
        ...
        if(p.getQuestStage(FAMILY_CREST) >= 4 && p.getQuestStage(FAMILY_CREST) <= 8) {
        ...
        if(p.getQuestStage(FAMILY_CREST) == 3) {
        ...
        if(p.getQuestStage(FAMILY_CREST) == 2) {
            ...
            if(!hasItem(p, 370) && !hasItem(p, 555) && !hasItem(p, 367) && !hasItem(p, 357) && !hasItem(p, 350)) {
                ...
                p.updateQuestStage(FAMILY_CREST, 3);
                int m = showMenu(p,n,
    ...
    private void HAVE_YOUR_BIT(Player p, NPC n) {
        ...
        if(menu == 0) {
            p.updateQuestStage(FAMILY_CREST, 2);
        } else if(menu == 1) {
