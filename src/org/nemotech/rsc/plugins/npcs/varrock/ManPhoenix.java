...
import static org.nemotech.rsc.plugins.Plugin.HEROS_QUEST;
...
TalkToNpcListener {
    ...
    public void onTalkToNpc(final Player p, final NPC n) {
        if(p.getCache().hasKey("arrav_gang") && p.getQuestStage(HEROS_QUEST) >= 1/* && p.getCache().getInt("arrav_gang") == PHOENIX_GANG*/) {
            if(hasItem(p, 585)) {
        ...
        if (!hasItem(p, 48) && p.getQuestStage(SHIELD_OF_ARRAV) == 5) {
        ...
        if (p.getInventory().hasItemId(49)
                && p.getQuestStage(SHIELD_OF_ARRAV) == 4) { // isnt this
            // going to
            // be 4?ya
            ...
            p.updateQuestStage(SHIELD_OF_ARRAV, 5);
            return;
        } else if (p.getQuestStage(SHIELD_OF_ARRAV) == 4) {
            npcTalk(p, n, "Hows your little mission going?");
        ...
        if (p.getQuestStage(SHIELD_OF_ARRAV) <= 3) {
            defaultConverstation(p, n);
            return;
        }
        if (p.getQuestStage(SHIELD_OF_ARRAV) == 5
                || p.getQuestStage(SHIELD_OF_ARRAV) == -1 || p.getQuestStage(HEROS_QUEST) == -1) {
    ...
    private void defaultConverstation(final Player p, final NPC n) {
        ...
        if (p.getQuestStage(Plugin.SHIELD_OF_ARRAV) == 3) {
                    ...
                    new Menu().addOptions(
                            new Option("I'd like to offer you my services") {
                                public void action() {
                                    ...
                                    p.updateQuestStage(SHIELD_OF_ARRAV, 4);
