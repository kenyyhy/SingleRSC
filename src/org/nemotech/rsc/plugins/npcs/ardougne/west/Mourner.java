...
public class Mourner implements TalkToNpcExecutiveListener, TalkToNpcListener {
    ...
    public void onTalkToNpc(Player p, NPC n) {
        if(n.getID() == MOURNER_444) {
            if(p.getQuestStage(Plugin.PLAGUE_CITY) != -1) {
            ...
            } else if(p.getQuestStage(Plugin.PLAGUE_CITY) == -1
                    && p.getQuestStage(Plugin.BIOHAZARD) == -1) {
        ...
        if(n.getID() == ILL_MOURNER) {
            if(p.getQuestStage(Plugin.BIOHAZARD) > 4) {
