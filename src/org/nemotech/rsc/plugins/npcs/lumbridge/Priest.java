...
import static org.nemotech.rsc.plugins.Plugin.THE_RESTLESS_GHOST;
...
public class Priest implements TalkToNpcExecutiveListener, TalkToNpcListener {
    ...
    public void onTalkToNpc(final Player p, final NPC n) {// that could work
        if (p.getQuestStage(THE_RESTLESS_GHOST) == 1) {
        ...
        if (p.getQuestStage(THE_RESTLESS_GHOST) >= 2) {
            ...
            } else if (p.getQuestStage(THE_RESTLESS_GHOST) == 3
                    && !hasItem(p, 27)) {
                playerTalk(
                        p,
                        n,
        ...
        if (p.getQuestStage(THE_RESTLESS_GHOST) == 0) {
            defaultMenu.addOption(new Option("I'm looking for a quest") {
                ...
                public void action() {
                    ...
                    p.updateQuestStage(THE_RESTLESS_GHOST, 1);
                }
            });
        }
