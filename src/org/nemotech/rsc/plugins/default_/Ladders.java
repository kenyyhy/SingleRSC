...
public class Ladders extends Plugin {
    ...
    public void onObjectAction(GameObject obj, String command, Player player) {
        ...
        if (obj.getID() == 487 && !Constants.MEMBER_WORLD) {
        ...
        //TelePoint telePoint = EntityHandler.getObjectTelePoint(obj.getLocation(), command);
        //if (telePoint != null) {
        ...
        if(obj.getID() == 487) {
            ...
            if(player.getX() == 567 && player.getY() == 3330) {
                player.getSender().sendTeleBubble(player.getX(), player.getY(), false);
            }
            return;
        }
        ...
        if(obj.getID() == 6 && obj.getX() == 282 && obj.getY() == 185 && !Constants.MEMBER_WORLD) {
            player.message(Constants.MEMBERS_ONLY_MESSAGE);
            return;
        }
        ...
        if (command.equals("climb-up") || command.equals("climb up")
                || command.equals("go up")) {
            int[] coords = coordModifier_(player, true, obj);
            ...
            int[] coords = coordModifier_(player, false, obj);
            player.teleport(coords[0], coords[1], false);
            player.message(
                    "You " + command.replace("-", " ") + " the "
                            + obj.getGameObjectDef().getName().toLowerCase());
    ...
    private int[] coordModifier_(Player player, boolean up, GameObject object) {
