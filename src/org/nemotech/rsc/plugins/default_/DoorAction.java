 ...
 * 
 */
public class DoorAction extends Plugin {
    ...
    //@Override
    public void onWallObjectAction(final GameObject obj, final Integer click, final Player p) {
        ...
        case 161:
            if(p.getQuestStage(GRAND_TREE) >= 8) {
                if(p.getX() >= 407) {
                    doDoor(obj, p);
            ...
            break;
        case 74:
            if (p.getQuestStage(HEROS_QUEST) == -1) {
                doDoor(obj, p);
            } else {
                NPC achetties = getNearestNpc(p, 253, 10);
                switch (p.getQuestStage(HEROS_QUEST)) {
                case 0:
                ...
                case 2:
                    ...
                    if (hasItem(p, 586) && hasItem(p, 590) && hasItem(p, 557)) {
                        ...
                        p.sendQuestComplete(HEROS_QUEST);
                    } else {
        ...
        case 109:
            if(p.getQuestStage(TEMPLE_OF_IKOV) >= 1 || p.getQuestStage(TEMPLE_OF_IKOV) == -1 || p.getQuestStage(TEMPLE_OF_IKOV) == -2) {
            ...
            break;
        case 108:
            if(p.getCache().hasKey("killedLesarkus") || p.getQuestStage(TEMPLE_OF_IKOV) == -1 || p.getQuestStage(TEMPLE_OF_IKOV) == -2) {
                p.message("You go through the door");
                doDoor(obj, p);
            } else {
            ...
            break;
        case 107:
            if(p.getCache().hasKey("completeLever") || p.getQuestStage(TEMPLE_OF_IKOV) == -1 || p.getQuestStage(TEMPLE_OF_IKOV) == -2) {
                p.message("You go through the door");
                doDoor(obj, p);
            } else {
            ...
            break;
        case 106:
            if(p.getCache().hasKey("openSpiderDoor") || p.getX() >= 536 || p.getQuestStage(TEMPLE_OF_IKOV) == -1 || p.getQuestStage(TEMPLE_OF_IKOV) == -2) {
                p.message("You go through the door");
                doDoor(obj, p);
            } else {
                p.message("The door won't open");
        ...
        case 105:
            if(p.getInventory().wielding(722) || p.getX() >= 546) {
                ...
                p.damage(Util.roundUp(p.getCurStat(3) / 5));
            }
            break;
            /** END **/
        case 9:
        ...
        case 114:
            if (p.getCache().hasKey("freed_servil")
                    || p.getQuestStage(FIGHT_ARENA) == -1
                    || p.getQuestStage(FIGHT_ARENA) == 3) {
                doDoor(obj, p);
                return;
            }
            ...
            break;
        case 122:
            if (p.getQuestStage(PLAGUE_CITY) >= 6
            || p.getQuestStage(PLAGUE_CITY) == -1) {
                if (p.getY() >= 569) {
                    doDoor(obj, p);
                    p.message("You go through the door");
                } else {
            ...
            if (ted != null) {
                ...
                if (p.getY() >= 569) {
                    if (removeItem(p, 768,1)) {
                        ...
                        doDoor(obj, p);
                        p.updateQuestStage(PLAGUE_CITY, 6);
                    }
                }
            }
            break;
        case 123:
            ...
            if (p.getQuestStage(PLAGUE_CITY) == 11
                    || p.getQuestStage(PLAGUE_CITY) == -1) {
                doDoor(obj, p);
                return;
            }
            if (p.getY() <= 605 || p.getY() >= 612) {
                ...
                if (mourner != null) {
                    ...
                    if (p.getQuestStage(PLAGUE_CITY) == 7) {
                        ...
                        if (menu == 0) {
                            ...
                            if (menu2 == 0) {
                                ...
                                p.updateQuestStage(PLAGUE_CITY, 8);
                            }
                        } else if (menu == 1) {
        ...
        case 121:
            ...
            if (p.getQuestStage(PLAGUE_CITY) >= 9
                    || p.getQuestStage(PLAGUE_CITY) == -1) {
                doDoor(obj, p);
                return;
            }
                ...
                if (p.getCurStat(10) < 68) {
                    ...
                    world.getDelayedEventHandler().add(
                            new ShortEvent(p) {
                                public void action() {
                                    owner.setBusy(false);
                                    owner.message("You need a fishing level of 68 to enter");
                                }
                            });
                } else {
                    doDoor(obj, p);
        ...
        case 22:// Odd looking wall
            p.getSender().sendSound(SoundEffect.SECRET_DOOR);
            doDoor(obj, p, -1);
        ...
        case 150: /* Fight Arena Ogre Cage */
            ...
            break;
        case 138:
            if(!p.getCache().hasKey("rotten_apples") && p.getQuestStage(BIOHAZARD) == 4) {
                message(p, "the door is locked",
                        "inside you can hear the mourners eating",
                        "you need to distract them from their stew");
            }
            else if(p.getCache().hasKey("rotten_apples") || p.getQuestStage(BIOHAZARD) == 5) {
                if(p.getY() <= 572) {
                    doDoor(obj, p);
                    p.message("you open the door");
                    p.message("You go through the door");
        ...
        case 141:
            ...
            break;
        case 145:
            if(p.getQuestStage(BIOHAZARD) == 7 || p.getQuestStage(BIOHAZARD) == 8 || p.getQuestStage(BIOHAZARD) == 9 || p.getQuestStage(BIOHAZARD) == -1) {
                if(p.getX() <= 82) {
                    p.message("You go through the door");
                    doDoor(obj, p);
                    return;
    ...
    //@Override
    public boolean blockInvUseOnWallObject(GameObject obj, InvItem item, Player player) {
        // Ernest the Chicken
        if (obj.getID() == 35) {
            return true;
        }
     ...
     */
    //@Override
    public void onInvUseOnWallObject(GameObject obj, InvItem item, Player player) {
    ...
    public boolean blockObjectAction(GameObject obj, String command, Player player) {
        if (obj.getGameObjectDef().getName().toLowerCase().contains("gate") || obj.getGameObjectDef().getName().toLowerCase().contains("door")) {
        ...
        case 57:
            if (obj.getX() == 435 && obj.getY() == 682) {
                if (!Constants.MEMBER_WORLD) {
                    player.message("You need to be a member to use this gate");
                    return;
                }
            }
    ...
    private void handleGates(GameObject obj, Player player) {
        ...
        case 611: // Shilo outside gate
            ...
            if(player.getQuestStage(SHILO_VILLAGE) == -1) {
        ...
        case 513: // eastern Varrock gate for family crest or biohazard or just wanna go in :)
            ...
            if (player.getX() >= 94) {
                if(player.getQuestStage(BIOHAZARD) == 7) {
        ...
        case 563: /* King Lanthlas Gate */
            ...
            if(player.getQuestStage(BIOHAZARD) == -1) {
                if(player.getY() <= 551) {
                    doGate(player, obj);
                } else {
                    if(lathasGuard != null) {
                        npcTalk(player,lathasGuard, "the king has granted you access to this training area",
                                "make good use of it, soon all your strength will be needed");
                    }
                    doGate(player, obj);
                }
                player.message("you open the gate");
        ...
        case 626: /* Gnome Stronghold Gate */
            if(player.getY() != 532 && player.getQuestStage(GRAND_TREE) == 8) {
                ...
                return;
            } else if(player.getY() >= 532 && player.getQuestStage(GRAND_TREE) == 10) {
                NPC n = getNearestNpc(player, 562, 15);
    ...
    private void replaceGameObject(final int newID, final boolean open,
        ...
        p.getSender().sendSound(open ? SoundEffect.OPEN_DOOR : SoundEffect.CLOSE_DOOR);
    }
